      ...
      <aside className="flex flex-col bg-[#1F202F] w-full">
                    ...
                    <PopoverContent className="w-fit px-2 py-0">
                        <div className="flex flex-col w-fit">
                            <div className="border-b py-2 flex flex-col">
                                ...
                                <span className="text-sm text-stone-500">Assignment 'Homework 4' is overdue.</span>
                            </div>
                            <div className="py-2 flex flex-col">
                                <span className="text-xs text-stone-400">Mon Apr 10, 6:47pm</span>
                                <span className="text-sm text-stone-500">Assignment 'Zybooks 11' is overdue.</span>
                            </div>
                        </div>
                    </PopoverContent>
                </Popover>
        ...
        <div className="flex items-center justify-between h-16 border-t border-[#34354A] p-4">
          <div className="flex flex-row gap-2">
              <Avatar>
                <AvatarImage alt="Anita Cruz" src="/placeholder.svg?height=32&width=32" />
                <AvatarFallback>{getInitials(user.name)}</AvatarFallback>
              </Avatar>
              <div className="flex flex-col">
                <span className="text-sm text-white">{loading ? "..." : user.name}</span>
                <span className="text-xs text-gray-400">{loading ? "..." : user.email}</span>
              </div>
          </div>
          <Popover>
            <PopoverTrigger><DotsIcon className="text-white h-4 w-4" /></PopoverTrigger>
