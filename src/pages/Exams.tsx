            ...
            <div className="flex flex-row gap-x-6">
                ...
                <Card className="p-4 basis-1/3 bg-[#f5f5f5]">
                    ...
                    <div className="flex flex-col">
                        {exams && exams.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((exam, i) => {
                            return new Date(exam.date).getTime() > new Date().getTime() ?
                                <a href={`/exams/${exam.id}`} className="hover:bg-[#ededed] transition-all duration-150">
                                    <div className={`${i === exams.length - 1 ? "" : "border-b"} border-stone-300 py-2 px-2`}>
                                        <p className="text-lg">{exam.title}</p>
                                        <div className="flex flex-row gap-x-2 items-center">
                                            <CoursesIcon size={16} />
                                            <p className="text-sm">{exam.courseSubject} {exam.courseNumber}: {exam.courseTitle}</p>
                                        </div>
                                        <div className="flex flex-row gap-x-2 items-center">
                                            <Calendar size={16} />
